...
### 📌 결정 2: Redisson 기반 분산 락(Distributed Lock)을 통한 동시성 제어
* **결정 내용**: 초고동시성 하의 재고 차감 로직의 무결성을 지키기 위해 Redis Lua Script 대신 **Redisson Distributed Lock**을 채택합니다.
* **결정 배경**: Redisson 분산 락은 DB 트랜잭션 전체를 감싸서 수행할 수 있어 **JPA 기반의 데이터 정합성(ACID) 관리가 직관적이며 오류 처리가 견고**합니다.
