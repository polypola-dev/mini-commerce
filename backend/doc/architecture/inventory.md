...
├── ExpiredReservationReleaser      만료 예약 해제 (스케줄 잡), 해제 성공 시 ReservationExpiredEvent 발행(3a)
└── ReservationExpiredEvent         재고 예약 만료 알림 이벤트 (order-infra가 in-process로 수신, 3a)
```
...
**리퍼→order EXPIRED 전이 완료(3a, 2026-07-03)**: 과거엔 리퍼가 재고만 풀고 주문 상태는 방치하는
갭이 있었는데, 지금은 `ExpiredReservationReleaser`가 release 성공 시 `ReservationExpiredEvent(reservationId,
orderId)`를 `ApplicationEventPublisher`로 발행하고, order-infra의 `ReservationExpiredEventListener`
(`@ApplicationModuleListener`)가 받아 order를 `EXPIRED`로 전이한다. order/inventory는 서비스그룹(b)
