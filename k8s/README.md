...
- `kind/cluster.yaml` — 로컬 kind 클러스터 정의 (G1, ADR-008), `kind/cluster-ci.yaml` — CI 전용 단일 노드 (G11, ADR-016)
- `base/` — 환경 무관 공통 매니페스트: 4개 서비스 Deployment+Service, ConfigMap (G2·G3, ADR-009)
- `overlays/local/` — kind 대상 오버레이 + 로컬 전용 postgres/redis StatefulSet+PVC (G4, ADR-010) + orderdb 생성 Job (G10, ADR-015), `overlays/ci/` — GitHub Actions 러너 보정(local 상속 + 부팅 예산 완화, G11, ADR-016), `overlays/prod/` — OKE 대상 오버레이
...
### 코드만 바뀐 뒤 재배포 (개발 inner-loop)

인프라·앱이 이미 떠 있고 **코드(이미지)만** 바꿨다면 위 전체 순서를 다시 할 필요 없다.
이미지 재빌드 → kind 주입 → 파드 롤아웃을 한 번에:

```bash
k8s/scripts/dev-redeploy.sh                    # 전체 서비스
k8s/scripts/dev-redeploy.sh order-api shop-api # 바뀐 서비스만 (빠름)
```

- 태그가 같은 `:latest`라 `kubectl apply -k`로는 파드가 안 바뀐다(스펙 동일) — 그래서
  `rollout restart`로 강제 교체해 새 이미지를 물린다.
- `order-api`가 대상에 있으면 먼저 롤아웃한다 — Flyway가 orderdb 스키마를 소유하므로
  validate-only인 `order-admin`/`order-batch`보다 먼저 떠 마이그레이션을 적용해야 한다.
- 대상 목록은 `overlays/local/kustomization.yaml`의 `images.newName`이 단일 출처
  (`build-and-load.sh`와 동일) — 빌드만 필요하면 `build-and-load.sh`, 롤아웃까지면 이 스크립트.
