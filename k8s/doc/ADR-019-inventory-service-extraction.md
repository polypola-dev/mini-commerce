...
- 상태: Accepted (2026-07-20), **경합 정책 개정 (2026-07-21)** — §4 참고
...
- **PG confirm 실패 시 release 안 함**(현행 유지) — PENDING_PAYMENT로 TTL 내 재결제 허용, 만료는
  ...
  창을 PG 호출 직전으로 좁힌다.
- **confirm 경합은 payment-wins, 단 재고가 실제로 남아있을 때만** (2026-07-21 개정 — 최초
  버전은 "순간 음수 재고 허용"이었으나, 리퍼가 풀어준 재고를 다른 주문이 즉시 재예약해가는
  경우 두 주문이 동일 재고를 동시에 CONFIRMED하는 실제 오버셀로 이어짐이 드러나 재설계했다):
  order.paid 컨슈머가 RELEASED/EXPIRED를 만나면 `forceConfirmScript`가 `reserveScript`와
  동일하게 **먼저 전 라인 재고를 검사한 뒤 통과해야 DECRBY**한다. 재고가 남아있으면 기존대로
  재차감+CONFIRMED 강제. 이미 다른 주문이 채간 상태면 DECRBY 없이 예약을 `OVERSOLD`로 표시하고
  `InventoryReservationOversoldEvent`를 발행(Kafka `inventory.reservation.oversold`) — 재고를
  거짓으로 확정하지 않는다. order-batch가 이를 구독해 `CancelOrderUseCase.cancelByAdmin`(기존
  PG 환불 배관 재사용)으로 해당 주문을 **정직하게 자동 취소+환불**한다. `restock()`은 OVERSOLD도
  RESTOCKED와 동일하게 no-op 처리(차감한 적이 없으므로 되돌릴 재고도 없음 — 안 하면 자동취소가
  발행하는 order.canceled 소비 시 IllegalState로 Kafka가 무한 재시도한다). 오버셀 발생은
  `inventory.reservation.oversold` Micrometer 카운터로 관측(초기 버전이 약속한 "메트릭"을 이때
  구현). 역방향(먼저 확정된 결제를 리퍼가 되돌리는 것)은 `markExpired()`의 PENDING_PAYMENT
  가드로 여전히 안전.
