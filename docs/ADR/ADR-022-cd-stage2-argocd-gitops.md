...
ksops Config Management Plugin 사이드카를 두고, `overlays/prod`의 KSOPS generator가
참조하는 `k8s/secrets/prod-app-secrets.enc.yaml`을 매니페스트 생성 시점에 복호화한다.
...
- **트레이드오프(수용)**: age 개인키가 클러스터(repo-server) 안에 산다 — 단, 키
  1개·한 곳이라 폭발 반경이 좁다. CMP 사이드카 설정이 Argo에서 가장 손이 가는 부분.
...
1. kind에서 App-of-Apps 리허설 — prod 오버레이를 kind context로 dry-run(`kubectl
   kustomize`), ksops CMP 복호화, wave 게이트 순서를 로컬에서 먼저 실증.
2. OKE에 부트스트랩(README 순서) → 전 wave Healthy → `/api` 200 + `/internal` 404
   + G9 차단 회귀(ADR-014, OKE CNI에서 재실행 필수 — 미지원 CNI면 조용히 무시).
