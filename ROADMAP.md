...
| # | P | 항목 | 근거/비고 |
|---|---|---|---|
| G1 | P0 | ✅ **완료(2026-07-13)** — 로컬 클러스터 **kind 확정(ADR-008, `docs/ADR/`)**: 표준 k8s(kubeadm)로 OKE와 동일 계열, GitHub Actions 재사용(G11 직결), arm64 네이티브. `k8s/kind/cluster.yaml`(control-plane 1 + worker 2, ingress-ready 라벨 + 80/443 매핑) 커밋, v1.34.0 클러스터 기동·워커 분산 스케줄링 스모크 검증 완료. **운영 클러스터는 OKE Basic(폴백 k3s)로 확정(ADR-007)** | G3 검증 시 compose 스택과 Docker VM 메모리(8G) 공유 주의 |
| G2 | P0 | ✅ **완료(2026-07-13)** — **Kustomize base/overlays 확정(ADR-009, `docs/ADR/`)**: `k8s/base` + `overlays/{local,prod}` 골격 커밋, kubectl 내장 kustomize 사용(`kubectl apply -k`, 별도 설치 없음). 네임스페이스 `mini-commerce` 단일, 공통 라벨 `part-of` base 일괄 부여, Helm은 외부 컴포넌트(G5 Strimzi·G6 ingress-nginx) 소비용으로 역할 분리. local 오버레이 kind 적용 검증 완료 | patch는 이미지·레플리카·리소스량으로 제한, 환경차는 env var 유지(F5 원칙) |
...
| G11 | P1 | 🔶 **1단계 완료(2026-07-14, 실전 검증 완료)** — **GitHub Actions kind 배포 검증(ADR-016)**: `.github/workflows/deploy-kind.yml` — kind(CI 단일노드 `cluster-ci.yaml`)+Strimzi+ingress-nginx 설치, compose 빌드→kind load, `overlays/ci`(local 상속+startupProbe 90회=180s+**cpu request 50m**, private 러너 2코어 보정) 배포, 롤아웃·orderdb-init 대기, 스모크(/api 200+시드, /internal 404, **G9 차단 회귀**). CI는 매번 빈 클러스터라 전체 부트스트랩(G10) 리허설을 매 실행 수행. Secret은 CI 더미(age 키 GH 미주입, ADR-013 정합). 트리거는 k8s/**·Dockerfile·compose 변경+수동(풀빌드 느려 매 푸시는 분수 낭비 — E5 후 재평가). **1차 실행에서 shop-api가 FailedScheduling(Insufficient cpu)으로 Pending 고착 발견** — 앱 4종+Kafka cpu request 합(950m)이 2코어(2000m) 러너 allocatable 초과. cpu limit이 원래 없는 설계(F6)라 request를 CI 전용으로 50m로 낮춰 해결(memory는 request=limit 유지 — JVM 힙 산정 기준이라 불변), Kafka는 kustomize 밖 리소스(`kubectl apply -f`)라 워크플로 내 `kubectl patch`로 별도 완화. **2차 실행 10분16초 전 스텝 성공**(kind 생성→Strimzi/ingress 설치→이미지 빌드/주입→배포→롤아웃→스모크→G9 회귀). **2단계(Argo CD) 설계 확정 + kind 실증 완료(2026-07-23, ADR-022, commit 89051c5~a546a14)** — App-of-Apps(`k8s/argocd/`) + sync-wave(operator/ingress 0→Kafka CR 1→앱 2, **Argo 미제공 Kafka health check(Lua)로 wave 2가 Kafka Ready 대기 → G3 부팅 크래시 방어**) + **Secret=ksops 채택**(ESO/OCI Vault 배제 — $0·솔로·SOPS/age 연속성, ADR-013 잔여결정 종결. 정본은 CMP 사이드카 아닌 **init container**로 ksops 내장 kustomize 설치) + 이미지태그 Git write-back(Image Updater 유보). **kind 실증**(Argo 차트 10.1.4/app v3.4.5): App-of-Apps·wave게이트(라이브 Kafka Healthy)·**ksops-in-sync**(sync 중 enc 복호화로 app-secrets 생성·관리)·**GitOps루프**(매니페스트 push→~45s 자동반영)·**selfHeal**(kubectl scale 되돌림 12s, 삭제 ConfigMap 재생성) 전부 통과. 🐛함정: StatefulSet VCT 기본값(immutable)→영구 OutOfSync가 앱 전체 selfHeal을 5분 스로틀 → `ignoreDifferences`로 해결(kind 전용 — prod엔 ArgoCD 관리 StatefulSet 없음). **잔여: OKE from-scratch 부트스트랩 + prod 오버레이 실체**(values-prod OCI LB / GHCR arm64 태그 / prod enc Secret / 버전 pin) — OKE 확보 대기. 매니페스트는 클러스터 무관 재사용(kubeconfig만 교체) | E4/E5가 빌드 스텝 개선의 선행. CI는 amd64 — arm64는 E4 소관. cpu request 50m는 CI 전용(overlays/ci) — local/prod는 원래 값(250m/100m) 유지 |
...
| H2 | P1 | ✅ **완료(2026-07-15, ADR-018)** — kube-prometheus-stack(monitoring ns). Prometheus **v3.13.1 네이티브 `enableOTLPReceiver`**로 compose(v2.55.1 experimental 플래그) 대비 단순화(README 트러블슈팅 미결 사항 해소). kind 컨트롤플레인 컴포넌트(scheduler/controller-manager/etcd/kube-proxy) 스크레이프는 kind 제약으로 비활성. 번들 Grafana에 Tempo/Loki datasource 추가 + **Ingress로 영구 접속**(`http://localhost/grafana`, `server.root_url`+`serve_from_sub_path` 서브패스 설정, G6 base ingress와 별도 공존) | k8s/observability/kube-prometheus-stack-values.yaml |
...
| H4 | P2 | ✅ **완료(2026-07-15, ADR-018)** — 서비스별 RED 대시보드(`$workload` 변수 — k8s Deployment 이름 도메인으로 4서비스 전환, 요청률·5xx 비율·p50/p95/p99·상태코드 분포·JVM 힙/GC·컨테이너 재시작·Pod age·Kafka 컨슈머 랙/멤버 수 총 10패널) ConfigMap(`grafana_dashboard: "1"`)으로 프로비저닝 — 기본 kubernetes-mixin 대시보드와 동일한 sidecar 메커니즘 재사용, 별도 설정 불필요. **버그 2건 발견·수정**(사용자가 k9s로 파드 강제 Kill 후 발견): JVM heap 라벨(`area`→`jvm_memory_type`), `$job`(OTel 도메인)이 kube-state-metrics 패널엔 안 먹히던 문제(→`target_info` 조인으로 `k8s_deployment_name` 통일) | k8s/observability/dashboards/ |
| H5 | P2 | ✅ **완료(2026-07-15, ADR-018)** — Alertmanager 활성화(`defaultRules.create: false` 유지, 기본 규칙 노이즈 배제) + `PrometheusRule` 3종(HighErrorRate 5xx>5%, PodRestartingFrequently 1시간 3회 초과, **KafkaConsumerLagHigh 1000건 초과**). **Kafka Exporter**를 Strimzi 내장 옵션(`spec.kafkaExporter`)으로 추가 — Service 없이 PodMonitor로 직접 스크레이프, NetworkPolicy는 Strimzi 자동생성 정책과 공존(ADR-014에서 이미 예상한 패턴, 변경 불필요). **receiver 미설정**(2026-07-15 사용자 결정: "지금은 규칙만" — Slack/Telegram webhook 준비되면 후속 작업) | k8s/observability/alert-rules.yaml, k8s/observability/kafka-exporter-podmonitor.yaml |
...
4. **k8s 전환**: ~~G1/G2(클러스터·매니페스트) → G3~G5(워크로드) → G8(Secret) → G6(Ingress) → G11 1단계(CI kind 배포) → H계열(관측성) → G11 2단계 Argo CD 설계+kind 실증(ADR-022)~~ ✅ 완료 — **잔여는 G7(HPA, P2)·G11 2단계의 OKE 실전(from-scratch 부트스트랩+prod 오버레이 실체, OKE 구축 선행)뿐**
