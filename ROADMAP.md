...
| H4 | P2 | ✅ **완료(2026-07-15, ADR-018)** — 서비스별 RED 대시보드(`job` 변수로 4서비스 전환, 요청률·5xx 비율·p50/p95/p99·상태코드 분포·JVM 힙/GC·파드 재시작) ConfigMap(`grafana_dashboard: "1"`)으로 프로비저닝 — 기본 kubernetes-mixin 대시보드와 동일한 sidecar 메커니즘 재사용, 별도 설정 불필요. Grafana API+datasource 프록시로 실데이터 확인 | k8s/observability/dashboards/ |
| H5 | P2 | ✅ **완료(2026-07-15, ADR-018)** — Alertmanager 활성화(`defaultRules.create: false` 유지, 기본 규칙 노이즈 배제) + `PrometheusRule` 2종(HighErrorRate 5xx>5%, PodRestartingFrequently 1시간 3회 초과). **컨슈머 랙은 kafka-exporter 부재로 스코프 제외**(캐치업 항목). **receiver 미설정**(2026-07-15 사용자 결정: "지금은 규칙만" — Slack/Telegram webhook 준비되면 후속 작업) | k8s/observability/alert-rules.yaml |
