...
| F6 | P1 | JVM 컨테이너 튜닝 — `-XX:MaxRAMPercentage` 기반 힙 설정, 서비스별 requests/limits 산정 (부팅 CPU burst 고려) | 무설정 시 OOMKill 단골 |
...
| G6 | P1 | Ingress 구축 — ingress-nginx, 라우팅 설계(`/api/**`→shop-api, `/api/orders`→order-api, `/api/admin/orders`→order-admin), CORS 정리 | 현재는 BFF가 포트별 직결 |
